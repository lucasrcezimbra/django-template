...
def test_django_migration_linter_included(cookies):
    """Test that django-migration-linter is included in dev dependencies."""
    result = cookies.bake()
...
def test_django_migration_linter_in_installed_apps(cookies):
    """Test that django_migration_linter is added to INSTALLED_APPS."""
    result = cookies.bake()
...
def test_makefile_includes_lintmigrations(cookies):
    """Test that the Makefile lint target includes lintmigrations command."""
    result = cookies.bake()
...
def test_migration_linter_works_with_different_configurations(cookies):
    """Test that migration linter is included regardless of configuration options."""
    configurations = [
